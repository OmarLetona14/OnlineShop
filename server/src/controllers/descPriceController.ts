...
class DescPriceController{

    async getDescPriceProducts(req:Request, res:Response){
        var cn = db.db2()
        ...
        order by p.price desc`,[],(result:any, err:any)=>{
            if (err) throw err;
            res.json(result)
        }); 
    }
...
const descpriceController = new DescPriceController()
export default descpriceController;